...
Ray ComputeCameraRay(int i, int j, int width, int height, Camera cam) {
  double top = cam.up.norm()/2;
  double bottom = -cam.up.norm()/2;
  double left = -cam.right.norm()/2;
  double right = cam.right.norm()/2;

  double us = left + (right - left)*((double)i+.5)/(double)width;
  double vs = bottom + (top - bottom)*((double)j+.5)/(double)height;
  double ws = cam.position(2) - 1;

  Eigen::Vector3f ray_direction = Eigen::Vector3f(us, vs, ws) - cam.position;
...
// /*   normalized_i = (2 * (i + 0.5) / (float)width - 1) * aspectRatio;
//    normalized_j = (1 - 2 * (j + 0.5) / (float)height);*/
